...
function Comic (options) {
  var _ = this._ = {};
  options = options || {};
  _.curl = options.httpClient || new net.HTTPClient();
}
...
Comic.prototype.fetchUpdates = function * fetchUpdates () {
  ...
  yield db_.addRefreshTasks(comicList);
};
...
Comic.prototype.fetchAll = function * fetchAll () {
  ...
  for (let i = 1; ; ++i) {
    console.info(i);
    let pageURL = getAllURLByPage(i);
    let all = yield this._.curl.getDOM(pageURL);
    all = all.querySelectorAll('.innr3 > table:nth-child(1) > tbody:nth-child(1) > tr > td:nth-child(1) > a:nth-child(1)');
    if (all.length <= 0) {
      break;
    }
...
Comic.prototype.process = function * process () {
  ...
  yield asyncio.forEach(tasks, function * (comic, index, self) {
    ...
    try {
      comic = yield * fetchComic(that, comic);
    } catch (e) {
...
function getAllURLByPage (n) {
  return `http://www.dm5.com/manhua-list-area36-lm3-s1-size60-p${n}/`
}
...
function * fetchComic (that, comic) {
